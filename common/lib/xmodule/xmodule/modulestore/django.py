...
from cm_plugin.xmodule_publish import xmodule_updated
...
def create_modulestore_instance(engine, content_store, doc_store_config, options, i18n_service=None):
    ...
    sig =Signal(providing_args=['modulestore', 'course_id', 'location'])
    #sig.connect(xmodule_updated, weak=False)
    
    return class_(
        contentstore=content_store,
        metadata_inheritance_cache_subsystem=metadata_inheritance_cache,
        request_cache=request_cache,
        modulestore_update_signal=sig,
