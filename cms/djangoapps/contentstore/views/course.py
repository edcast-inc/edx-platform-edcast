    ...
    if response_format == 'json' or 'application/json' in request.META.get('HTTP/ACCEPT','application/json'):
        if request.method == 'POST' or request.method == 'PUT' or request.method == 'PATCH':
            ...
            try:
                ...
                log.error("course creation attempted by unregistered user: %s", instructor_email)
...
def cm_update_course(request, course_id):
    ...
    try:
        ...
        org = course_id_parts[0]
        number = course_id_parts[1]
        run = course_id_parts[2]
        course_old_location = Location('i4x', org, number, 'course', run)
	course_key = SlashSeparatedCourseKey(org, number, run)
        ...
        if 'start_date' in jsondict:
            ...
            converted = None
        if converted != None and converted != descriptor.start:
            dirty = True
        ...
        if converted != None and converted != descriptor.end:
            dirty = True
...
def cm_create_new_course(request):
    try:
        ...
        if 'url' in response_body:
            course_id = request.json.get('org')+'/'+ \
            ...
            content = {'errors':response_body['ErrMsg']}
