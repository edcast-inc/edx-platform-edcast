...
from cm_plugin import gradebook
    ...
    for student, gradeset, err_msg in iterate_grades_for(course_id, enrolled_students):
        ...
        if gradeset:
            ...
            for label in header:
                ...
                if score != 0.0:
                    gb_record, gb_record_created = CmGradebookRecords.objects.get_or_create(
                        cm_gradebook=cm_gradebook,
                        user_email=student.email,
                        unit_name=label
                    )
                    gb_record.score = score
                    gb_record.save()
        else:
