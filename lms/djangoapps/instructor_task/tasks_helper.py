...
def upload_grades_csv(_xmodule_instance_args, _entry_id, course_id, _task_input, action_name):
    ...
    page = int(_task_input['page'])
    limit = int(_task_input['limit'])
    status_interval = 100
    ...
    lower_bound = min((total_students), limit * page)
    upper_bound = min((total_students), (limit * (page +  1)))
    
    num_total = 0
    if not page == -1:
      enrolled_students = enrolled_students[lower_bound:upper_bound]
      num_total = len(enrolled_students)
    else:
